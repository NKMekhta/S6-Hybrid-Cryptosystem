...
impl GFP2Element {
    pub fn get_random_coefficients(prime: BigInt) -> (BigInt, BigInt) {
        loop {
            let a = thread_rng()
                .gen_bigint_range(&BigInt::from(0), &BigInt::from(prime.clone().sub(1)));
            let b = thread_rng()
                .gen_bigint_range(&BigInt::from(0), &BigInt::from(prime.clone().sub(1)));
            if a.clone().ne(&b) {
                return (a, b);
    ...
    pub fn get_pow_2(&self) -> Self {
        ...
        Self::new_with_coefficients(self.prime.clone(), (b * (b - 2 * a), a * (a - 2 * b)))
    }
...
impl Add for GFP2Element {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        let (ref xa, ref xb) = self.coefficients;
        let (ref ya, ref yb) = rhs.coefficients;
        Self::new_with_coefficients(self.prime.clone(), (xa + ya, xb + yb))
    }
}
